...
* **🖼️ Fix image/PDF attachments dropped on `/responses` (#154)**: The `/responses` adapter forwarded Chat-Completions content parts (`image_url`, `file`) verbatim, but the Responses API — and LiteLLM's bridge for Anthropic/Bedrock/Vertex/Gemini — only understands `input_text` / `input_image` / `input_file`, so vision prompts silently lost their screenshots on bridged providers and were malformed for native OpenAI. Each user/assistant turn now becomes exactly ONE `message` item whose content is translated part-by-part into Responses-native types (all-text turns keep LiteLLM's plain-string shortcut; `cache_control` stamps survive on the translated part). The body also now sends `max_output_tokens` — the Responses-API name for the output cap — instead of the chat-only `max_tokens`, which LiteLLM filtered as unknown (so the cap was never enforced on this route), and stops emitting `frequency_penalty` / `presence_penalty` / `stop` / `stream_options` for the same reason. The `cache_control`-rejection retry now strips stamps from inside `/responses` content parts. (`src/adapters/responsesAdapter.ts`, `src/adapters/litellmClient.ts`, `src/utils/promptCacheControl.ts`, `src/types.ts`)
...
* **🤖 Claude Fable 5.1 / Mythos 5.1 compatibility (#146)**: Four breaking changes on `claude-fable-5-1` and `claude-mythos-5-1` no longer cause hard failures or silent regressions. **Forced `tool_choice`** (`ToolMode.Required`) is now downgraded to `"auto"` before the request is sent — both endpoints were affected because LiteLLM translates OpenAI's `{type:"function", function:{name}}` into Anthropic's `{type:"tool"}`, which these models reject with a 400. **Front-trimming now strips retained `thinking_blocks`** ("keep-tail compaction"): Fable 5.1 thinking blocks are bound to the conversation prefix that produced them, so replaying them after the trimmer removed earlier messages would fail with "The block is bound to a different conversation" on enforced accounts. **The one-shot continuity retry** now also recognizes the Fable 5.1-specific rejection strings (`bound to a different conversation`, `prefix_binding_mismatch`, `model_binding_mismatch`) instead of relying on generic `"invalid" + "thinking block"` overlap. **`stop_reason: "refusal"`** — which arrives with HTTP 200 — is now detected on both endpoints and logged instead of being treated as an ordinary completion. (`src/providers/base/requestBuilder.ts`, `src/providers/base/thinkingBlockRetry.ts`, `src/providers/base/parameterFiltering.ts`, `src/adapters/streaming/liteLLMStreamInterpreter.ts`, `src/adapters/tokenUtils.ts`)
* **🎚️ Strip sampling params for Fable 5.1 via static denylist**: `claude-fable-5-1` and `claude-mythos-5-1` were added to `KNOWN_PARAMETER_LIMITATIONS` (`temperature`, `top_p`, `top_k` — these models 400 on non-default values), covering proxies whose model cards omit or misreport `supported_openai_params`. Explicit model-card lists still win over the static fallback. (`src/providers/base/parameterFiltering.ts`)
* **🧭 Unify Fable 5.1 model-ID matching across all guards**: The forced-`tool_choice` downgrade and the sampling-param denylist previously matched model IDs differently — the boundary regex missed snapshot and Bedrock aliases (`claude-fable-5-1@20260801`, `anthropic.claude-fable-5-1-v1:0`, `us.anthropic.…`), while the substring denylist over-matched a hypothetical `claude-fable-5-10`. Both guards now share a single boundary-aware `isFable51Family()` helper in `src/utils/modelUtils.ts`, so an aliased Fable 5.1 can never get its sampling params stripped while a forced `tool_choice` (a hard 400 on these models) still goes out. A parity test enforces the two guards agree, the enforcing mock rejects the aliased shapes on the wire, and conservative over-matching (`claude-fable-51`) is retained since a false positive only costs the soft downgrade while a missed alias hard-fails. (`src/utils/modelUtils.ts`, `src/providers/base/requestBuilder.ts`, `src/providers/base/parameterFiltering.ts`, `src/test/integration/mockLiteLLMBackend.ts`)
* **🧪 Replace third-party JUnit reporters with an in-repo, version-agnostic reporter**: CI crashed with `TypeError: Class constructor Base cannot be invoked without 'new'` because `mocha-junit-reporter@2.2.1` and `mocha-multi-reporters@1.5.1` construct mocha's Base reporter as a plain function invocation — legal on mocha 11 (function-style Base) but a hard error on mocha 12 (released 2026-08-31), where Base is an ES class. Since `package-lock.json` is deliberately volatile (rebuilt on demand), CI's fresh resolution could pull the new major at any time. The new `JunitSpecReporter` (`src/test/junitSpecReporter.ts`) extends mocha's built-in `Spec` — a class `extends` clause constructs correctly whether the parent is mocha 11's function-style or mocha 12's class-style reporter — and writes the JUnit XML itself, re-rendering the full document on every suite completion so a crashed run still leaves a parseable artifact for the Codecov upload. Verified end-to-end against a real mocha 12.0.0 installation (construction, totals, failure escaping, skipped handling) and against mocha 11 in the full suite. (`src/test/junitSpecReporter.ts`, `src/test/junitSpecReporter.test.ts`, `.vscode-test.mjs`, `package.json`)
